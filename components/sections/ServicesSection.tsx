...
export function ServicesSection() {
  ...
  const services = [
    {
      titleEn: "Parent Counseling",
      titleAm: "የወላጆች ምክር",
      descEn: "Compassionate guidance and emotional support for parents navigating unique developmental challenges.",
      descAm: "ልዩ የዕድገት ፈተናዎችን ለሚያሳልፉ ወላጆች ርህራሄ የተሞላበት መመሪያ እና ስሜታዊ ድጋፍ።",
    ...
    {
      titleEn: "Spiritual Care",
      titleAm: "መንፈሳዊ እንክብካቤ",
      descEn: "Faith-based encouragement anchoring families in the truth of the Gospel and lasting peace.",
      descAm: "ቤተሰቦችን በወንጌል እውነት እና በዘላቂ ሰላም የሚያጸና በእምነት ላይ የተመሠረተ ማበረታቻ።",
    ...
    },
    {
      ...
      titleAm: "አካታች ትምህርት",
      descEn: "Tailored educational assistance designed to help children reach their full divine potential.",
      descAm: "ልጆች መለኮታዊ አቅማቸው ላይ እንዲደርሱ ለማገዝ የተዘጋጀ የተስተካከለ የትምህርት ድጋፍ።",
    ...
    },
    {
      ...
      titleAm: "የማኅበረሰብ ውህደት",
      descEn: "Providing safe, inclusive environments where families belong and are truly valued.",
      descAm: "ቤተሰቦች የሚካተቱበት እና እውነተኛ ክብር የሚያገኙበት አስተማማኝ እና አካታች ቦታዎችን ማዘጋጀት።",
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="max-w-3xl mb-12">
          <span className="inline-block py-1.5 px-5 rounded-full bg-white dark:bg-wisdom-surface border border-slate-200 dark:border-white/10 text-wisdom-blue font-bold text-xs sm:text-sm tracking-[0.2em] uppercase mb-4 shadow-sm">
            {am ? "የእኛ አገልግሎቶች" : "What We Offer"}
          </span>
          <h2 className="font-heading font-extrabold text-3xl sm:text-4xl lg:text-5xl text-wisdom-text tracking-tight leading-tight">
            {am ? (
              <>
                ተግባራዊ ድጋፍ እና <br className="hidden sm:block" />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-wisdom-blue via-wisdom-green to-wisdom-yellow">መንፈሳዊ ጥንካሬ.</span>
              </>
            ) : (
              <>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, i) => (
            <div key={i} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2.5rem] p-8 hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 group">
              ...
              <h3 className="font-heading font-bold text-xl text-wisdom-text mb-4">{am ? service.titleAm : service.titleEn}</h3>
              <p className={`text-sm text-wisdom-muted leading-relaxed font-medium ${am ? 'font-amharic' : ''}`}>{am ? service.descAm : service.descEn}</p>
            </div>
          ))}
        </div>
