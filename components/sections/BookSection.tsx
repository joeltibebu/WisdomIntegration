        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
          ...
          <div className="lg:col-span-12 xl:col-span-5 relative group flex justify-center lg:justify-start">
            ...
            <div className="relative z-10 w-full max-w-[320px] lg:max-w-[380px]">
              ...
              />
              <div className={`absolute -bottom-6 -right-6 w-32 h-32 bg-wisdom-yellow rounded-full flex items-center justify-center text-wisdom-text font-black text-xs uppercase tracking-widest p-4 text-center shadow-lg border-4 border-white dark:border-wisdom-bg -rotate-12 group-hover:rotate-0 transition-transform ${am ? 'font-amharic' : ''}`}>
                {am ? "የእውነተኛ ህይወት ጉዞ" : "Real Life Journey"}
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-12 xl:col-span-7 space-y-8">
            <div>
              <span className="inline-block py-1.5 px-5 rounded-full bg-white dark:bg-wisdom-surface border border-slate-200 dark:border-white/10 text-wisdom-orange font-bold text-xs sm:text-sm tracking-[0.2em] uppercase mb-4 shadow-sm">
                {am ? "ራዕይ እና ግንዛቤ" : "Visionary Insights"}
              </span>
              <h3 className="font-heading font-extrabold text-3xl sm:text-4xl lg:text-5xl text-wisdom-text tracking-tight leading-tight">
                {am ? (
                  <>
                    ከእኛ ጉዞ <br className="hidden sm:block" />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-wisdom-blue to-wisdom-green">ወደ ዎኝ.</span>
                  </>
                ) : (
                  <>
            ...
            <div className="space-y-6">
              <p className={`text-wisdom-muted text-lg sm:text-xl leading-relaxed font-medium max-w-2xl ${am ? 'font-amharic' : 'font-body'}`}>
                {am 
                  ? "ከእውነተኛ ህይወት ልምድ የተጻፈው ይህ መጽሐፍ፣ ልዩ ፍላጎት ያለው ልጅን በማሳደግ የተገኙ ጉዞዎችን፣ ፈተናዎችን እና ጥበብን ያካፍላል — ለተመሳሳይ ቤተሰቦች ተስፋ እና መመሪያ ይሰጣል።" 
