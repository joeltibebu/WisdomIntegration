...
export default async function EditChildPage({ params }: EditChildPageProps) {
  ...
  if (!child) notFound();
  if (child.parentId !== session.user.id) redirect("/dashboard/parent/children");

  // Format dateOfBirth as YYYY-MM-DD for the date input
  const dateOfBirth = child.dateOfBirth.toISOString().split("T")[0];
    ...
    <DashboardShell role="PARENT" userName={session.user.name}>
      <div className="space-y-6 max-w-2xl">
        <div>
          ...
          <h1 className="text-2xl font-heading font-bold text-wisdom-text">Edit Child Profile</h1>
          <p className="text-sm text-wisdom-muted mt-1">Update {child.name}&apos;s information.</p>
        </div>
        <ChildProfileForm
          initialData={{
            id: child.id,
