...
export const metadata: Metadata = {
  ...
  description:
    "Bible-based devotionals and messages of faith to anchor your heart in the love of Christ. Find strength and peace for your journey.",
};
    ...
    <div className="bg-white dark:bg-white/5 p-12 lg:p-16 rounded-[3rem] border border-slate-100 dark:border-white/10 shadow-sm relative overflow-hidden group">
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        <div className="text-center space-y-4">
          <h3 className="font-heading font-extrabold text-3xl text-wisdom-text">
            {titleEn}
          </h3>
          <p className="font-amharic text-xl font-bold text-wisdom-blue">
            {titleAm}
          </p>
...
export default async function SpiritualFoodPage() {
  let posts: {
    id: string;
    title: string;
    slug: string;
  ...
  try {
    posts = await prisma.contentPost.findMany({
      where: { published: true, category: "spiritual-food" },
  ...
  try {
    videos = await prisma.video.findMany({
      where: { is_published: true, category: "spiritual-food" },
  ...
  const devotionals: DevotionalProps[] = [
    {
      ...
      contentEn:
        "He gives strength to the weary and increases the power of the weak. In the moments when our own hands feel heavy, we are reminded that our true source of power is not within us, but above us.",
      contentAm:
        "ለደከመው ኃይልን ይሰጣል፤ ጉልበት ለሌለውም ብርታትን ይጨምራል፡፡ የእኛ እጆች በከበዱበት ቅጽበት፣ የእውነተኛው የጥንካሬ ምንጫችን ከእኛ ውስጥ ሳይሆን ከላይ መሆኑን እናስታውሳለን፡፡",
    },
    {
      ...
      contentEn:
        "And the peace of God, which transcends all understanding, will guard your hearts and your minds in Christ Jesus. Peace is not the absence of trials, but the presence of the Father in the middle of them.",
      contentAm:
        "ከማስተዋልም ሁሉ በላይ የሆነው የእግዚአብሔር ሰላም ልባችሁንና አሳባችሁን በክርስቶስ ኢየሱስ ይጠብቃል፡፡ ሰላም ማለት የፈተናዎች አለመኖር ሳይሆን፣ በፈተናዎች መካከል የአባት መገኘት ነው፡፡",
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-wisdom-bg">
      ...
      {/* Static Devotionals — Wisdom for the Day */}
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-20">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          ...
          <h2 className="font-heading font-extrabold text-3xl text-wisdom-text">
            Wisdom for the Day
          </h2>
          <p className="text-wisdom-muted font-medium text-lg italic uppercase tracking-widest opacity-60">
            Daily Bilingual Reflections
          </p>
        </div>
